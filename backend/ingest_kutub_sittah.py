...
def populate_kutub_sittah():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    total_added = 0

    for col in COLLECTIONS:
        print(f"\n--- Ingestion de {col['name']} (Limite: {col['limit']}) ---", flush=True)
        ...
        conn.commit()
        print(f"SUCCESS: {count} Hadiths insérés pour {col['name']} !", flush=True)
        total_added += count

    print(f"\n🎉 TOTAL INGÉRÉ : {total_added} Hadiths mis à jour et indexés dans la base SQLite (sunnah.db) !", flush=True)
    conn.close()

if __name__ == "__main__":
    populate_kutub_sittah()
